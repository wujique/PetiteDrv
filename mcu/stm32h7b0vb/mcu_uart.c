...
static s32 mcu_uart_receive(McuUartNum comport, u8 *buf, s32 len);
...
void mcu_uart4_irq(void)
{
		...
		&& (__HAL_UART_GET_IT_SOURCE(&McuHuart4, UART_IT_RXNE) != RESET))
    {
		...
		mcu_uart_receive(MCU_UART_4, &ch,1);
...
s32 mcu_uart_open(McuUartNum comport)
{
	...
	else if(comport == MCU_UART_4)
	{
		...
		McuUart[MCU_UART_4].Buf = McuUartLogBuf;
		mcu_MX_UART4_Init();
...
/*
	将接收到的数据填入缓冲
*/
s32 mcu_uart_receive(McuUartNum comport, u8 *buf, s32 len)
{
	s32 i;
	...
	while(1)
	{
		*(McuUart[comport].Buf + McuUart[comport].End) = *buf;

		McuUart[comport].End++;
		if(McuUart[comport].End >= McuUart[comport].size) 
			McuUart[comport].End = 0;
