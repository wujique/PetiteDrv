...
s32 dev_nrf24l01_init(void)
{
	...
	/* ce */
	mcu_io_config_out(RF24L01_CE_PORT, RF24L01_CE_PIN);
	
	/* IRQ */
	mcu_io_config_in(RF24L01_IRQ_PORT, RF24L01_IRQ_PIN);
	
	mcu_io_output_resetbit(RF24L01_CE_PORT, RF24L01_CE_PIN);
	return 0;
}
...
static s32 dev_nrf24l01_setce(u8 sta)
{
	if(sta == 1)
	{
		mcu_io_output_setbit(RF24L01_CE_PORT, RF24L01_CE_PIN);	
	}
	else
	{
		mcu_io_output_resetbit(RF24L01_CE_PORT, RF24L01_CE_PIN);	
...
{
	u8 sta;
	sta = mcu_io_input_readbit(RF24L01_IRQ_PORT, RF24L01_IRQ_PIN);	
	if(sta == MCU_IO_STA_1)
		return 1;
	else
		return 0;
