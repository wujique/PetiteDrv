...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	mp3DecInfo_pointer = (MP3DecInfo *)pvPortMalloc(sizeof(MP3DecInfo));//mymalloc(SRAMIN,sizeof(MP3DecInfo));
	if (!mp3DecInfo_pointer)
		return 0;
	ClearBuffer(mp3DecInfo_pointer, sizeof(MP3DecInfo));
	
	fh =  (FrameHeader *)     pvPortMalloc(sizeof(FrameHeader));//mymalloc(SRAMIN,sizeof(FrameHeader));
	si =  (SideInfo *)        pvPortMalloc(sizeof(SideInfo));//mymalloc(SRAMIN,sizeof(SideInfo));
	sfi = (ScaleFactorInfo *) pvPortMalloc(sizeof(ScaleFactorInfo));//mymalloc(SRAMIN,sizeof(ScaleFactorInfo));
	hi =  (HuffmanInfo *)     pvPortMalloc(sizeof(HuffmanInfo));//mymalloc(SRAMIN,sizeof(HuffmanInfo));
	di =  (DequantInfo *)     pvPortMalloc(sizeof(DequantInfo));//mymalloc(SRAMIN,sizeof(DequantInfo));
	mi =  (IMDCTInfo *)       pvPortMalloc(sizeof(IMDCTInfo));//mymalloc(SRAMIN,sizeof(IMDCTInfo));
	sbi = (SubbandInfo *)     pvPortMalloc(sizeof(SubbandInfo));//mymalloc(SRAMIN,sizeof(SubbandInfo));
