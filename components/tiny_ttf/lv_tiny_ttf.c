...
#define TTF_CACHE_REALLOC(x,y)  (wjq_realloc(x,y))
#define TTF_CACHE_FREE(x)    (wjq_free(x))
#define TTF_MALLOC(x)  (wjq_malloc(x))
#define TTF_FREE(x)    (wjq_free(x))

lv_fs_res_t ttf_fs_open(lv_fs_file_t * file_p, const char * path, lv_fs_mode_t mode)
{
	//lv_fs_open(file_p, path, LV_FS_MODE_RD);
	
	file_p->file_d = (void *)vfs_open(path, "wr");

	return 0;
}

int ttf_fs_read(lv_fs_file_t * file_p, void * buf, uint32_t btr)
{
	uint32_t br;
    //lv_fs_read(file_p, buf, btr, &br);

	br = vfs_read((int)(file_p->file_d), buf, btr);
	
	return 0;
}
int ttf_fs_seek(lv_fs_file_t * file_p, uint32_t pos, lv_fs_whence_t whence)
{
	//lv_fs_seek(file_p, pos, whence);
	vfs_lseek((int)(file_p->file_d), pos, whence);
	return 0;
}
...
static void * ttf_cache_add(ttf_cache_handle_t handle, int key, int size)
{
    ...
    for(int i = 0; i < bucket->capacity; ++i) {
        ttf_cache_entry_t * entry = &bucket->entries[i];
        if(entry->age == 0) {
            entry->data = (void *)TTF_CACHE_MALLOC(size);
            if(entry->data == NULL) {
                return NULL;
            }
            entry->size = size;
    ...
    void * result = (void *)TTF_CACHE_MALLOC(size);
...
{
    if(stream->file != NULL) {
		ttf_fs_read(stream->file, data, to_read);
    }
    else {
...
static void ttf_cb_stream_seek(ttf_cb_stream_t * stream, size_t position)
{
    if(stream->file != NULL) {
		...
		ttf_fs_seek(stream->file, position, 0);
    }
    else {
...
#if LV_TINY_TTF_FILE_SUPPORT !=0
    if(path != NULL) {
        if(LV_FS_RES_OK != ttf_fs_open(&dsc->file, path, LV_FS_MODE_RD)) {
...
/*
		By default, a font will use up to 4KB of cache to speed up rendering glyphs. 
		This maximum can be changed by using lv_tiny_ttf_create_data_ex(data, data_size, line_height, cache_size) 
		or lv_tiny_ttf_create_file_ex(path, line_height, cache_size) (when available).
		The cache size is indicated in bytes.
*/
