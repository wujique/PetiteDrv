...
fdb_err_t _fdb_init_ex(fdb_db_t db, const char *name, const char *path, fdb_db_type type, void *user_data)
{
    ...
    if (db->file_mode) {
        ...
        block_size = fal_partition_getblksize(db->storage.part);;//fal_flash_device_find(db->storage.part->flash_name)->blk_size;
        if (db->sec_size == 0) {
        ...
        db->max_size = fal_partition_getlen(db->storage.part);///db->storage.part->len;
